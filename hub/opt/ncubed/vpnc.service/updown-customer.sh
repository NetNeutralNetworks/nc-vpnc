...
NETNS=${PLUTO_CONNECTION}              # outputs c0001
...
up-client)
    ...
    # add routes
    ip -n ${TRUSTED_NETNS} -6 route add ${V6_CUST_TUNNEL_SPACE}::/96 via ${V6_CUST_TUNNEL_SPACE}:1:0:1
    ip -n ${NETNS} route add 0.0.0.0/0 dev ${XFRM}
    # start NAT64
    ip netns exec ${NETNS} jool instance add --netfilter --pool6 ${V6_CUST_TUNNEL_SPACE}::/96
    ;;
down-client)
